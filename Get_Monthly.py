...
class EurostatScraper:
    ...
    def get_demand_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            dff_euro['date'] = pd.to_datetime(dff_euro['variable'], format="%Y-%m", errors="coerce")
            
            
            # Filter for consumption data only
            consumption = dff_euro[dff_euro['nrg_bal'] == 'TI_EHG_MAP']
...
if __name__ == "__main__":
    ...
    df.iloc[:, :4].to_csv(r"C:\Users\RoryPerkins\OneDrive - Timera Energy\Documents\eurostat_monthly.csv", index=False)